    ...
    public void algorithm(HashSet<Team> teamList, Season season) {
        ...
        for(Team team : teamList) {
            if(team != guest){
                ...
                createGameAfterChecks(season.getLeague(), season.getYear(),date, field, team.getName(), guest.getName(), array[refereesToGame[0]], array[refereesToGame[1]], array[refereesToGame[2]],Integer.parseInt(season.getYear()),month,day);
                day += 7;
                if (day > 28) {
                    day = 1;
                    month++;
